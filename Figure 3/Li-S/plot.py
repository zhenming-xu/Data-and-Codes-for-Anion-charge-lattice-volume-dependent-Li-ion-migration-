...
def hm_barrier():
    ...
    barrier = barrier.reshape(14, 14)
    volume_factor = []
    volume = []
    for i in range(108, 135, 2):
    ...
    fig, ax = plt.subplots()
    im = ax.imshow(barrier, cmap=plt.cm.Greens)
    ...
    ax.set_title("charge-volume-energy barrier (Li-S)")
    plt.setp(ax.get_xticklabels(), rotation=45, ha="right",
             rotation_mode="anchor")
    cbar = ax.figure.colorbar(im, ax=ax)
    cbar.ax.set_ylabel("Energy barrier / eV", rotation=-90, va="bottom")
    plt.savefig("energy_barrier_Li-S.png", bbox_inches='tight')
    plt.show()
...
def hm_tet():
    ...
    ax.set_xlabel("Volume / " + r"$\AA ^ 3$")
    ax.set_ylabel("Charge / e")
    ax.set_title("charge-volume-energy of site_tet (Li-S)")
    plt.setp(ax.get_xticklabels(), rotation=45, ha="right",
             rotation_mode="anchor")
    cbar = ax.figure.colorbar(im, ax=ax)
    cbar.ax.set_ylabel("Energy of site_tet / eV", rotation=-90, va="bottom")
    plt.savefig("tet_energy_Li-S.png", bbox_inches='tight')
    plt.show()
